      ...
      <div className={`group absolute flex flex-col h-[calc(100%_-_4rem)] w-16 bg-white right-0 hover:w-40 transition-all duration-300`}>
        ...
        {props.accounts.map((account, index) => (
          ...
          <div className="flex flex-none w-full h-16 bg-slate-200 border-b border-slate-300 items-center justify-center" key={index}>
            ...
            <div className="absolute w-10 h-10 bg-slate-300 rounded-full right-[0.75rem] group-hover:transition transition group-hover:duration-300 duration-300">
              {account.profile_pic !== '' ?
                <Image src={account.profile_pic} alt='' width={640} height={640} priority className="object-contain rounded-full" /> :
                <Image src={profilePic} alt='' priority className="object-contain rounded-full" />
              }
            </div>
          </div> :
